...
if(datoGenerico === null) {
  ...
  const valoreRisolto = await datoGenerico;
  console.log(valoreRisolto);
}

else if(typeof datoGenerico === "string") {
  const datoInMaiuscolo = datoGenerico.toUpperCase(); 
  console.log(datoInMaiuscolo); 
}
else if(typeof datoGenerico === "number") {
  const risultato = datoGenerico * 2;
